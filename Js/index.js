...
const mockData = {
    items: [
        { 
            volumeInfo: { title: "Senhor dos Anéis", authors: ["J.R.R. Tolkien"], averageRating: 4.9, categories: ["Fantasia"], imageLinks: { thumbnail: "https://via.placeholder.com/128x192/2c3e50/ffffff?text=Senhor+dos+Aneis" } },
...
async function fetchBooks() {
    // 1. Criamos uma lista de palavras bem amplas em inglês para trazer resultados misturados
    const palavrasChave = ['world', 'history', 'science', 'ocean', 'space', 'future', 'art', 'magic', 'life'];
    
    // 2. Sorteamos uma palavra dessa lista
    const palavraSorteada = palavrasChave[Math.floor(Math.random() * palavrasChave.length)];

    // 3. Sorteamos um número de 0 a 40 para pular os primeiros resultados. 
    ...
    try {
        ...
        books.forEach(book  => {
            ...
            const categories = info.categories ? info.categories.join(", ") : "Sem categoria";
